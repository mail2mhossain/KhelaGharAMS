...
namespace KhelaGhar.AMS.Model.Repository
{
    ...
    public class AsarRepository : AbstractFactoryAndRepository
    {
        ...
        public IList<Asar> ByCommitteeType(string committeeType)
        {
            ...
            if (committeeType == Committee.TypeOfCommittee.পূর্ণাঙ্গ.ToString())
            {
                asars = (from a in Container.Instances<AsarCommittee>()
                         where (int)a.Committee.CommitteeType == (int)Committee.TypeOfCommittee.পূর্ণাঙ্গ
                         select a.Asar).ToList();
            }
            else if (committeeType == Committee.TypeOfCommittee.আহ্বায়ক.ToString())
            {
                asars = (from a in Container.Instances<AsarCommittee>()
                         where (int)a.Committee.CommitteeType == (int)Committee.TypeOfCommittee.আহ্বায়ক
                         select a.Asar).ToList();
            }
            else
            {
